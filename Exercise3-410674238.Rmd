...
```{r ans12}
a<-classData[classData$`公-10-06`=="有看公告",]
```
...
```{r ans14}
b<-classData[classData$`公-10-06`=="沒看公告",]
sum(b$`練2018-10-17-1`==1)/nrow(b)
```
...
```{r ans21}
library(lubridate)
ymd_hms(hwData$Timestamp, tz="Asia/Taipei")->hwData$Timestamp
```
...
```{r ans25}
aa<-hwData[(hwData$Timestamp<deadline & hwData$作業代碼=="hw1"),]
table(aa$early) 
```